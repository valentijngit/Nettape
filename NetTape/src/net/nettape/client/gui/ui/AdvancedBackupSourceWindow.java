...
public class AdvancedBackupSourceWindow extends AbstractFilesLocaleView {
	...
	public AdvancedBackupSourceWindow(BackupSet backupSet) {
		this();
	...
	public AdvancedBackupSourceWindow() {
		super(Language.getTextAdvancedBackupSourceWindow(Language.ADVANCED_BACKUP_SOURCE_WINDOW_TEXT_TITLE),
				Language.getTextAdvancedBackupSourceWindow(Language.ADVANCED_BACKUP_SOURCE_WINDOW_TEXT_EXPLAIN),
			AppImages.getImage16(AppImages.IMG_FOLDER), AppImages.IMG_BACKUP_SET);
		try {
			this.createGUI();
		}
		catch (Exception e) {
	...
	private boolean setGrayed(File file) {
		return this.treeFiles.get(file.getAbsolutePath().toLowerCase()).getGrayed();
	}
	...
	private void checkAllParents(File file) {
		...
		while (StringUtil.isNotEmpty(parent)) {
			...
			for (int i = 0; i < files.length; i++) {
				if ((this.treeFiles.get(files[i].getAbsolutePath().toLowerCase()) == null)
						|| (!this.treeFiles.get(files[i].getAbsolutePath().toLowerCase()).getChecked())) {
					nr++;
				}
				if ((this.treeFiles.get(files[i].getAbsolutePath().toLowerCase()) != null)
						&& this.treeFiles.get(files[i].getAbsolutePath().toLowerCase()).getGrayed()) {
					nrGray++;
				}
			}
			FileState fS = this.treeFiles.get(fileParent.getAbsolutePath().toLowerCase());
			if (fS == null) {
				fS = new FileState();
			...
			this.treeFiles.put(fS.getFile().getAbsolutePath().toLowerCase(), fS);
	...
	protected boolean buttonOKAction() {
		...
		try {
			if(this.backupSet.backupsetitems != null)
				this.backupSet.backupsetitems.clear();
			if(this.backupSet.filters != null)
				this.backupSet.filters.clear();
			...
			while (iter.hasNext()) {// remove all file unchecked or grayed
				...
				if (!fileS.getChecked() || (fileS.getChecked() && fileS.getGrayed())) {
					this.treeFiles.remove(fileS.getFile().getAbsolutePath().toLowerCase());
				}
			}
			...
			while (iter.hasNext()) {
				...
				item.setPath(fileS.getFile().getAbsolutePath().toLowerCase());
				item.setIsfolder(fileS.getFile().isDirectory());
				item.setRoot(FileUtil.getRoot(fileS.getFile().getAbsolutePath().toLowerCase()));
				backUpSetItems.add(item);
			}
			this.backupSet.backupsetitems = backUpSetItems;
			if(this.setFilters != null)
			{
				...
				while (iterFilters.hasNext()) {
					...
					filter.setBackupset(this.backupSet.backupset);
					hashNew.add(filter);
				}
				this.backupSet.filters = hashNew;
			}
			result = true;
		}
		catch (Exception e) {
	...
	private void applyFiltersToTree()
	{
		...
		Iterator<String> iter = this.treeFiles.keySet().iterator();

		while (iter.hasNext()) 
		{
			// remove all files that apply to filters of type exclude
			// add files that apply to filter of type include
			FileState fileS = this.treeFiles.get(iter.next());
			for(Filter filter : this.setFilters)
			{
				if(filter.getInclude())
				{
					Iterator<Criteria> criteriaIter = filter.getCriterias().iterator();
					while(criteriaIter.hasNext())
					{
						Criteria criteria = criteriaIter.next();
						
						if(filter.getApplytofiles())
						{
							if ((!fileS.getChecked()) && fileS.getFile().isFile() 
									&& (fileS.getFile().getAbsolutePath().matches(criteria.getPattern()))
									&& (fileS.getFile().getAbsolutePath().indexOf(filter.getToppath()) == 0)) 
	    					{
								fileS.setChecked(true);
							}
						}
						if(filter.getApplytofolders())
						{
							if ((!fileS.getChecked()) && fileS.getFile().isDirectory() 
									&& (fileS.getFile().getAbsolutePath().matches(criteria.getPattern()))
									&& (fileS.getFile().getAbsolutePath().indexOf(filter.getToppath()) == 0)) 
	    					{
								fileS.setChecked(true);
	...
	private boolean setCheck(File file) {
		FileState fState = this.treeFiles.get(file.getAbsolutePath().toLowerCase());
		if (fState == null) {
			return false;
		}
	...
	private void addAllChildren(File file, boolean checked) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			this.treeFiles.put(files[i].getAbsolutePath().toLowerCase(), fState);
		}
	}
	...
	private void addFileToHash(final File file, final boolean checked) {
		...
		fState.setFile(file);
		fState.setChecked(checked);
		this.treeFiles.put(file.getAbsolutePath().toLowerCase(), fState);
	}
	...
	private void checkFileChildren(final File file, final boolean state) {
		...
		while (key.hasNext()) {
			String string = key.next().toLowerCase();
			String str = string + File.separator;
			ArrayList<String> strings = StringUtil.splitStrByDelim(string, File.separator);
			ArrayList<String> keys = StringUtil.splitStrByDelim(file.getAbsolutePath().toLowerCase(),
					File.separator);
			if ((string.indexOf(file.getAbsolutePath().toLowerCase()) != -1) && (strings.size() > keys.size())) {

				// if ((str.indexOf(file.getAbsolutePath()) != -1)
	...
	private void checkTreeItem(TreeItem item) {
		...
		FileState fS = this.treeFiles.get(file.getAbsolutePath().toLowerCase());
		if (fS == null) {
			fS = new FileState();
			fS.setFile(file);
		}
	...
	public void populateData() {
		try {
			...
			for (int i = 0; i < items.length; i++) {
				...
				fS.setChecked(false);
				fS.setGrayed(false);
				this.treeFiles.put(file.getAbsolutePath().toLowerCase(), fS);
			}

			this.setFilters = this.backupSet.filters;
			Iterator<Backupsetitem> iterBackUp = this.backupSet.backupsetitems.iterator();
			while (iterBackUp.hasNext()) {
				...
				while ((parent != null) && parent.exists()) {
					...
					fS.setChecked(true);
					fS.setGrayed(true);
					this.treeFiles.put(parent.getAbsolutePath().toLowerCase(), fS);
