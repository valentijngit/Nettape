...
public class RestoreWindow extends AbstractFilesRemoteView {
	...
	private Button buttonDeleteExtraFiles;
	...
	public RestoreWindow(Restore restore, Set<Backup> backUp) {
		this();
	...
	public RestoreWindow() {
		super(Language.getTextRestoreWindow(Language.RESTORE_WINDOW_TEXT_TITLE),
			AppImages.getImage16(AppImages.IMG_FOLDER_PREVIOUS));
		try {
			createGUI();
			populateData();
		} catch (Exception ex) {
	...
	protected boolean buttonOKAction() {
		...
		try {
			...
			for (int j = 0; j < listResult.size(); j++) {
				...
				item.setRestore(this.restore);
				item.setDatetime(Calendar.getInstance().getTime());
				item.setPath(n.getPath());
				items.add(item);
			}
			this.restore.setRestoreitems(items);
			this.restore.setDatetime(Calendar.getInstance().getTime());
			this.restore.setRestorepermissions(this.buttonRestoreFile.getSelection());
			if (this.filter != null) {
				this.filter.setRestore(this.restore);
			}
			this.restore.setFilter(this.filter);
			result = true;
		} catch (Exception e) {
			result = false;
	...
	private boolean buttonFilterAction() {
		RestoreFilterWindow dlg = new RestoreFilterWindow(this.filter);
		dlg.open();
		if (dlg.getExitChoiceAction() == SWT.OK) {
			this.filter = dlg.getResult();
		}
		return false;
	}
	...
	protected void createBottom() {
		...
		this.buttonDeleteExtraFiles = new Button(compDelete, SWT.CHECK);
		data = new GridData(SWT.RIGHT, SWT.FILL, true, true);
		this.buttonDeleteExtraFiles.setLayoutData(data);
		this.buttonDeleteExtraFiles.setText(Language.getTextRestoreWindow(Language.RESTORE_WINDOW_BUTTON_DELETE_EXTRA_FILE));
	...
	public void populateData() {
		try {
			...
			if (this.restore != null) {
				this.buttonRestoreFile.setSelection(this.restore.getRestorepermissions());
				this.filter = this.restore.getFilter();
			}
			...
			Iterator<Backup> iterBackup = this.backups.iterator();
			int index = 0;
			while (iterBackup.hasNext()) {
				Backup backup = iterBackup.next();
				this.comboDate.add(backup.getName());
				if (backup.getBackupset() == null) {
					continue;
				}
				Iterator<Backupsetitem> iter = backup.getBackupset().getBackupsetitems().iterator();
				while (iter.hasNext()) {
					Backupsetitem itm = iter.next();
					addItemsToMap(index, itm);
				}
				index++;
			}
			if ((this.restore != null) && (this.restore.getRestoreitems() != null)) {
				Iterator<Restoreitem> restoreItems = this.restore.getRestoreitems().iterator();
	...
	private void addItemsToMap(int index, Backupsetitem item) {
		try {
